...
| GUI の画面/設定 | `app.py`（単一ファイル・Flask + インラインHTML/CSS/JS、**UI変更フロー厳守**）。生成画面は2ステップ・ウィザード（①対象設定: URL＋クロールモード＋深さ/最大300＋詳細設定[auth] ②出力形式）→ 実行ビュー（ライブプレビュー＝`/api/live-screenshot` でクロール中スクショをポーリング・内包）→ **結果ページ（QAビュー軸の4タブ）**。クロールモードは単体／オートクローリング（`/api/discover`→選択）／手動URL追加。結果配信は `/api/result`（生成物一覧＋サマリー）`/preview`（iframe・テキスト）`/download`・`/download-zip`（出力配下のみ許可のパスガード付き）。 |
| 結果ページのタブ | ①概要（画面インベントリ＋遷移先＋スクショ）②**入力項目・テスト条件マトリクス**（全画面横断・画面/型/必須フィルタ・検索・CSV書き出し。QAのテスト設計ワークシート）③画面別仕様（`report.html` 埋め込み）④エクスポート（各形式DL＋ZIP）。データ源は `report.json`（GUI実行時は常に生成）を `/preview` で取得しクライアントで構築。 |
