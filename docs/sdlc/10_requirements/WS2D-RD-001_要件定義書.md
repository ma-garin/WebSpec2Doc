...
業務フローの詳細な図（画面遷移を含むアクティビティ図相当）は `WS2D-BF-001`（業務フロー図、本書執筆時点で未整備・今後作成予定）に譲り、本書では概要のみを記載する。GUIの標準利用フローは次の4ステップであり、システム化範囲全体を横断する。
...
- 業務フロー詳細図（`WS2D-BF-001`）は本書執筆時点で未整備。次回改訂までに作成し本書7章から参照する予定。
