...
| GUI の画面/設定 | `app.py` はエントリーポイント（Flask + `web/` パッケージ）。**4ステップウィザード**（①解析: URL入力＋画面分析 → ②条件設定: 画面選択＋個別ログインフォーム＋差分ON/OFF → ③実行: ライブプレビュー＋4段階進捗バー → ④レポート）。`/api/discover` で画面一覧取得（タイムアウト180秒）、認証必要画面は per-page インラインフォームで個別ログイン（`/api/login/simple`）。結果配信は `/api/result`（生成物一覧＋サマリー）`/preview`（iframe・テキスト）`/download`・`/download-zip`（パスガード付き）。静的ファイルは `static/app.js`＋`static/app-view.js`＋`static/app.css` の3分割。 |
| 結果ページのタブ | ①概要（画面インベントリ）②画面別仕様（スクショ+フォームカード+テスト条件。フォームなし画面はページ要素一覧）③**入力項目・テスト条件マトリクス**（全画面横断・フィルタ・CSV書き出し）④設計（技法推奨マトリクス）⑤**技法詳細**（画面別・技法別サンプル値カード）⑥**画面遷移図**（Mermaid UML 3サブタブ: シーケンス図/コミュニケーション図/アクティビティ図）⑦遷移表（リンク先URLパス付き）⑧履歴・差分。エクスポートはヘッダーのドロップダウン（HTML/PDF/JSON/Excel/MD×2/Mermaid/差分/ZIP）。データ源は `report.json`（GUI実行時は常に生成）を `/api/result` で取得しクライアントで構築。 |
...
python -m pytest tests/ -q                       # 273本 緑が基準
...
| 画面遷移図が表示されない | Mermaid v10 を CDN(jsdelivr)で動的ロード。オフライン環境では表示されない。3サブタブ（シーケンス/コミュニケーション/アクティビティ）で切替可能。 |
