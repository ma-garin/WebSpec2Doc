...
| TTFV: URL→レポート表示 | discover 2.36s ＋ クロール 16.61s ＝ **約19s** | ~~現状比 −30%（約13s）~~ **2026-07-30 取り下げ**（下記） | P0-1 |
...
### TTFV −30% の取り下げ（2026-07-30・オーナー判断）

**達成できないと分かった目標を残さない。** 他の判断の基準として機能しないため。

- P2-6 の調査で、**クロール高速化では −30% に届かない**ことが判明した
  （`docs/design/2026-07-29_crawl-perf-investigation.md`）。固定費 0.8 秒＋1 画面 2.4 秒のうち、
  削減余地が最大のアクション探索でも −19%、並列4で −7%、礼儀待ちの短縮は 0 秒。
  **残り約 1.9 秒/画面は成果物を作る処理そのもの**で、削ると出力の質が落ちる。
- 代替として実施した 2 件は受入を満たしている。
  - **P1-1 残り時間の精度**: 誤差の中央値 **+26%**（受入 ±40% 以内）
  - **P3-1 ゼロ待ちサンプル**: ボタン押下から表示まで **0.11 秒**（実解析は約 21 秒）
- 新しい数値目標は**置かない**。速度が問題として再発した時点で、そのとき測って設定する。
...
**P0-3 エラー経路の棚卸し**（見積 1h）— ✅ **完了（2026-07-29）**
- 結果: `docs/design/2026-07-29_fetch-error-audit.md`。`static/js/*.js` の `fetch(` **87 経路**を点検し、
  無言で固まる 3 経路を修正。silent 0 / no_catch 0（意図的除外 3）で受入を満たす
- 点検は `scripts/check_fetch_error_handling.py` として残してあり、以降も再実行できる
...
**P1-1 クロール残り時間の動的表示**（見積 2h）— ✅ **完了（2026-07-30）／受入 達成**

**残課題も解消した（2026-07-30）**
- 成果物の生成フェーズ（全体の約25%）を残り時間に含めた。`src/main.py` から
  `generate_started` / `generate_step` / `generate_completed` を配信し、
  7 工程（遷移品質・影響範囲・文書突合・UX/A11y・スナップショット・差分・書き出し）を区切る
- 画面側は 1 工程あたりの実測時間から生成フェーズの残りを見積もり、クロール分に足す。
  実績が溜まるまでは 0.8 秒/工程の初期値を使う
- 進捗バーも生成フェーズぶん（76%→96%）を割り当て、どこで待っているかが分かるようにした
- **実測（実 UI・3 回）: 誤差の中央値 +26%**（受入 ±40% 以内）。
  従来はクロール終了時点で「まもなく完了」と出したまま 5〜6 秒待たせていた
...
- 変更0件のときは「**前回から変更はありません（比較: &lt;from&gt; → &lt;to&gt;）**」を明示する。
  判定用に `GET /api/snapshot-diff-summary` を新設し、変更件数を機械可読な形でも配信する
  （差分レポート本体は HTML のため、画面側から 0 件かどうかを判定できなかった）。
  件数が取れなかったときは何も言わない（不在を断定しない）
...
2. ~~遷移図×テストケース 2 ペインのデザイン破綻修正（main に混入中・修正 5 案作成済み）~~
   → ✅ **対応済み（2026-07-29）**。実測: 図が左ペインの**面積の 7%** しか占めず 93% が空白なのに、
   右のテストケース表は 378px に詰まっていた。左右比を固定（1.2fr / .8fr）していたのが原因のため、
   図の側を実寸に合わせる（`fit-content(58%)`）ようにした。結果、テストケース表は **378px → 613px**
   となり URL が折り返さなくなった。図の拡大はしない（小さい図を引き伸ばすと線と文字だけ太って
   読みにくくなるという既存の判断を尊重）。1440px / 1280px で横スクロールが出ないことを確認
