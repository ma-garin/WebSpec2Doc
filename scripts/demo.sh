...
DEMO_SITE_V2_PORT="${DEMO_SITE_V2_PORT:-8767}"
...
DEMO_SITE_PID=$!
# site_v2（新バージョン相当）も併走起動する（移行検証デモ用・SPEC-3-5）。
# 8766=現行サイトは不変のまま、8767 で新側を配信する。
"${PYTHON}" "${ROOT}/demo/demo_site.py" --port "${DEMO_SITE_V2_PORT}" --site-dir site_v2 &
DEMO_SITE_V2_PID=$!
trap 'kill "${DEMO_SITE_PID}" "${DEMO_SITE_V2_PID}" 2>/dev/null || true' EXIT
...
for _ in $(seq 1 20); do
  if curl -s -o /dev/null "http://127.0.0.1:${DEMO_SITE_V2_PORT}/"; then
    break
  fi
  sleep 0.5
done
...
echo ""
echo "  デモ対象サイト（現行） : http://127.0.0.1:${DEMO_SITE_PORT}/"
echo "  デモ対象サイト（新）   : http://127.0.0.1:${DEMO_SITE_V2_PORT}/"
echo "  WebSpec2Doc            : http://127.0.0.1:8765/"
echo ""
echo "  → 本体の URL 欄に http://127.0.0.1:${DEMO_SITE_PORT}/ を入力してください"
echo "  → 移行検証デモ: --compare-old-urls に現行、--compare-new-urls に新側を指定"
