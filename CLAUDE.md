...
| 変更種別 | 必須コマンド | 省略時の結果 |
|---------|------------|------------|
| Python (`*.py`) | `make test` | pre-commit hook がブロック |
| UI (`*.html` / `*.js` / `*.css`) | `make test` **AND** `make verify-ui` | pre-commit hook がブロック |
| UI 変更後コミット前 | ブラウザ目視確認必須 | DoD 違反・インシデント扱い |

> **根拠 (INC-2026-001)**: pytest PASS は L1/L2 の確認のみ。  
> L3（E2E）・L4（受け入れ）なしでの UI コミットは品質保証の放棄に等しい。  
> `make verify-ui` なしの HTML/JS/CSS コミットは **pre-commit hook により強制ブロック**される。

## 作業場所（重要）

```
/Users/fujimagariyuki/Desktop/app/014_WebSpec2Doc  # macOS 開発機
/home/user/WebSpec2Doc                              # Linux 環境（CI / Codex 等）
```
> 環境依存のパスは適宜変更してください。
- 旧 `app_趣味/008_CrateSpec2HTML` は**廃止**（venv 破損・参照禁止）。
- セッションは必ずこのディレクトリで開く（Codex 等のランタイムも環境に合わせて固定される）。
...
│   └── llm/            ← LLM連携（実装済み：screen_classifier / viewpoint_generator / industry_template）
...
│       ├── events.js            ← イベントハンドラ
│       ├── file-preview.js      ← ファイルプレビュー
│       ├── recrawl.js           ← 再クロール
│       ├── settings.js          ← 設定ビュー
│       ├── qa-tools.js          ← QAツールビュー
│       ├── view-shots.js        ← スクリーンショットタブ
│       ├── view-utils.js        ← ビューユーティリティ
│       ├── view-export-settings.js ← エクスポート設定タブ
│       ├── review.js            ← レビュービュー
│       └── traceability.js      ← トレーサビリティビュー
├── templates/
│   ├── index.html      ← HTML骨格（partials を include）
│   └── partials/       ← 15 partial（view-generate/dashboard/auto-run/qa-* etc.）
├── tests/              ← pytest（956本・src/web コア90%+）
...
make setup-hooks                 # ← 必須: pre-commit hook をインストール（品質ゲート）
```

> `make setup-hooks` を省略すると UI 変更の品質ゲートが機能しない（INC-2026-001 再発リスク）。
...
- 現行の API設定は **OpenAI**（ANTHROPIC ではない）。src/llm/ の基本モジュールは実装済み（screen_classifier / viewpoint_generator / industry_template）。
- 旧 008 ディレクトリは使わない。

## 現在地と次

- 完了: クローラー / ドリフト検知 / 自動ログイン(GUI) / GUI多画面化 / テストベースレポート / M1②PDF出力 / M1③JSON出力 / M2 Phase B（ロケータ候補列・クロール条件メタ） / 4ステップウィザード / 個別ログインフォーム / 技法詳細タブ / Mermaid UML 3サブタブ＋テスト観点マップ / エクスポートドロップダウン / コードベース分割（app.js→21モジュール・qa_process→4モジュール・index.html→15 partial） / ダッシュボード再設計（差分バッジ・鮮度ラベル・スナップショット数）/ サイト削除機能 / CSRF強化（urlparse netloc完全一致） / サイドバー折りたたみ＋最大化ボタン / レポート8タブ化（テスト条件・技法詳細・遷移表を独立タブ化） / 差分バッジ・タブカウントバッジ / スレッドセーフ対応（_JOBS_LOCK）/ src/llm/ 基本モジュール実装（screen_classifier / viewpoint_generator / industry_template）/ テスト956本・コア90%+。
- 次: LLMテスト観点（src/llm/ OpenAI統合・完成度向上）→ M3 連携（TestRail/Jira）。
