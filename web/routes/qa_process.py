...
def api_qa_process_generate() -> dict | tuple[dict, int]:
    ...
    if report is None:
        return {"error": "invalid report.json"}, 400
    ai_status: dict[str, Any] = {"requested": use_ai, "available": has_openai_api_key(), "used": False, "fallback": False}
    ai_artifact = None
    if use_ai:
        if not ai_status["available"]:
            ai_status |= {"fallback": True, "error": "OPENAI_API_KEY が設定されていないためテンプレート生成に切り替えました。"}
        else:
            try:
                ai_artifact = generate_openai_qa(domain, report, _load_qa_viewpoints())
                _ai_artifact_path(domain).write_text(json.dumps(ai_artifact, ensure_ascii=False, indent=2), encoding="utf-8")
...
def _output_payload(domain: str) -> dict[str, str]:
    ...
    return {key: _existing_path(out_dir / filename) for key, _label, filename in QA_STEPS}
...
def _generate_outputs(domain: str, report: dict[str, Any], ai_artifact: dict[str, Any] | None = None) -> dict[str, Path]:
    out_dir = OUTPUT_DIR / domain / "qa_process"
    out_dir.mkdir(parents=True, exist_ok=True)
    generated_at = datetime.now().strftime("%Y-%m-%d %H:%M")
    docs = _docs_from_ai_artifact(domain, ai_artifact, generated_at) if ai_artifact else {
    ...
    for key, _label, filename in QA_STEPS:
        ...
        if key == "qa_process_report":
            path.write_text(_qa_process_report_html(domain, report, docs, generated_at, bool(ai_artifact)), encoding="utf-8")
        else:
...
def _docs_from_ai_artifact(domain: str, artifact: dict[str, Any] | None, generated_at: str) -> dict[str, str]:
    if not artifact:
        return {}
    plan = artifact.get("test_plan", {}) if isinstance(artifact.get("test_plan"), dict) else {}
    analysis = artifact.get("test_analysis", {}) if isinstance(artifact.get("test_analysis"), dict) else {}
    design = artifact.get("test_design", {}) if isinstance(artifact.get("test_design"), dict) else {}
    cases = artifact.get("test_cases", {}) if isinstance(artifact.get("test_cases"), dict) else {}
    review = artifact.get("cross_review", {}) if isinstance(artifact.get("cross_review"), dict) else {}
    report = artifact.get("qa_process_report", {}) if isinstance(artifact.get("qa_process_report"), dict) else {}
    return {
        "test_plan": "\n".join(
            [
                f"# テスト計画: {domain}",
...
def _analysis_table(analysis: dict[str, Any]) -> str:
    ...
    for item in analysis.get("source_inventory", []) if isinstance(analysis.get("source_inventory"), list) else []:
...
def _risk_table(analysis: dict[str, Any]) -> str:
    ...
    for item in analysis.get("risk_items", []) if isinstance(analysis.get("risk_items"), list) else []:
        if isinstance(item, dict):
            rows.append(f"| {_md(item.get('risk_id', ''))} | {_md(item.get('description', ''))} | {_md(item.get('impact', ''))} | {_md(item.get('trace_id', ''))} |")
    return "\n".join(rows)
...
def _design_table(design: dict[str, Any]) -> str:
    ...
    for item in design.get("viewpoints", []) if isinstance(design.get("viewpoints"), list) else []:
        if isinstance(item, dict):
            rows.append(f"| {_md(item.get('viewpoint_id', ''))} | {_md(item.get('target', ''))} | {_md(item.get('technique', ''))} | {_md(item.get('design_note', ''))} | {_md(item.get('trace_id', ''))} |")
    return "\n".join(rows)


def _coverage_table(design: dict[str, Any]) -> str:
    rows = ["| Trace | Covered By | Coverage Note |", "|---|---|---|"]
    for item in design.get("coverage_matrix", []) if isinstance(design.get("coverage_matrix"), list) else []:
        if isinstance(item, dict):
            rows.append(f"| {_md(item.get('trace_id', ''))} | {_md(item.get('covered_by', ''))} | {_md(item.get('coverage_note', ''))} |")
    return "\n".join(rows)


def _cases_table(cases: dict[str, Any]) -> str:
    rows = ["| ケースID | タイトル | 手順 | 期待結果 | 実行区分 | 状態 | Trace |", "|---|---|---|---|---|---|---|"]
    ...
    for screen in _screens(report):
        ...
        for form_idx, form in enumerate(_forms(screen), 1):
            ...
            for field_idx, field in enumerate(_fields(form), 1):
                ...
                lines.append(f"| TD-{trace} | 入力項目 `{_md(field.get('name') or field.get('element_id') or 'unnamed')}` | {_md(cond)} | {trace} |")
    for idx, viewpoint in enumerate(_viewpoints_by_type("category_l2")[:12], 1):
        lines.append(f"| TD-VP-{idx:02d} | {_md(viewpoint['name'])} | CSV観点を対象仕様へ適用し、該当有無・期待結果・不足仕様を確認 | QA-VP-{idx:02d} |")
    lines += ["", "## 質問待ち", "- 業務上の同値クラス、境界値、禁止文字、重複登録条件は仕様確認が必要です。", ""]
    return "\n".join(lines)


def _test_cases(domain: str, report: dict[str, Any]) -> str:
    lines = [f"# テストケース: {domain}", "", "| ケースID | 種別 | 手順 | 期待結果 | Trace |", "|---|---|---|---|---|"]
    case_no = 1
    for screen in _screens(report):
        sid = _sid(screen)
        title = screen.get("title") or screen.get("url") or sid
        lines.append(f"| TC-{case_no:04d} | 画面表示 | `{_md(screen.get('url', ''))}` を開く | `{_md(title)}` の画面仕様が表示される | {sid} |")
        case_no += 1
        for to_id in _transitions_to(screen):
            lines.append(f"| TC-{case_no:04d} | 画面遷移 | {sid} から {to_id} へ遷移する操作を実行 | 遷移先画面へ到達する | {sid}->{to_id} |")
            case_no += 1
        for btn_idx, button in enumerate(_buttons(screen), 1):
            trace = f"{sid}-B{btn_idx:02d}"
            lines.append(f"| TC-{case_no:04d} | 操作要素 | `{_md(button)}` を操作 | 期待結果は質問待ち。少なくともエラーなく応答する | {trace} |")
            case_no += 1
        for form_idx, form in enumerate(_forms(screen), 1):
            for field_idx, field in enumerate(_fields(form), 1):
                trace = _field_trace_id(screen, form_idx, field_idx)
                label = field.get("name") or field.get("element_id") or field.get("placeholder") or "unnamed"
                lines.append(f"| TC-{case_no:04d} | 入力 | `{_md(label)}` に代表値を入力 | 入力値が受理される | {trace} |")
                case_no += 1
                if field.get("required"):
                    lines.append(f"| TC-{case_no:04d} | 必須 | `{_md(label)}` を未入力にする | 必須エラーが表示され送信されない | {trace} |")
                    case_no += 1
                for cond in field.get("test_conditions") or []:
                    lines.append(f"| TC-{case_no:04d} | 条件 | `{_md(label)}` で {_md(cond)} | 仕様通りに受理またはエラー表示される | {trace} |")
                    case_no += 1
    for idx, viewpoint in enumerate(_viewpoints_by_type("category_l2")[:10], 1):
        lines.append(f"| TC-{case_no:04d} | 観点レビュー | CSV観点 `{_md(viewpoint['name'])}` を対象仕様へ照合 | 該当仕様、非該当理由、不足質問が記録される | QA-VP-{idx:02d} |")
        case_no += 1
    if case_no == 1:
        lines.append("| TC-0001 | 質問待ち | 画面仕様の詳細確認 | テスト可能な期待結果を定義する | QA-UNKNOWN |")
    return "\n".join(lines) + "\n"
...
def _cross_review(domain: str, report: dict[str, Any]) -> str:
    ...
    if summary["fields"] and summary["required"] == 0:
        findings.append("- 入力項目はありますが必須項目が検出されていません。HTML属性と業務必須の差分確認が必要です。")
    if summary["transitions"] == 0 and summary["screens"] > 1:
        findings.append("- 複数画面がありますが遷移情報がありません。リンク抽出条件の確認が必要です。")
    if not findings:
        findings.append("- 自動抽出範囲では重大な欠落兆候はありません。期待結果と業務ルールは質問待ちです。")
    csv_viewpoints = "\n".join(f"- {name}" for name in _viewpoint_names("quality_area_l1", 16)) or "- 質問待ち"
    return f"# 横断レビュー: {domain}\n\n## レビュー観点\n- 画面網羅性\n- 入力項目網羅性\n- 必須/任意の妥当性\n- 操作要素と遷移の整合性\n- トレーサビリティIDの付与\n\n## CSV横断観点\n{csv_viewpoints}\n\n## 指摘\n" + "\n".join(findings) + "\n\n## 質問待ち\n- 画面ごとの優先度と利用頻度\n- 障害時の業務影響\n- 非機能要件と監査観点\n"


def _qa_process_report_html(
    domain: str, report: dict[str, Any], docs: dict[str, str], generated_at: str, used_ai: bool = False
) -> str:
    summary = _qa_summary(report)
    screen_rows = "".join(
...
def _screens(report: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [screen for screen in screens if isinstance(screen, dict)] if isinstance(screens, list) else []
...
def _buttons(screen: dict[str, Any]) -> list[str]:
    ...
    return [str(button) for button in buttons if str(button).strip()] if isinstance(buttons, list) else []
...
def _transitions_to(screen: dict[str, Any]) -> list[str]:
    ...
    return [str(to_id) for to_id in to_ids if str(to_id).strip()] if isinstance(to_ids, list) else []
