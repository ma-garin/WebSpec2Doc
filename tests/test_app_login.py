...
def test_scrape_returns_fields(tmp_path: Path, monkeypatch) -> None:
    ...
    payload = json.dumps({"ok": True, "fields": [{"name": "username", "field_type": "text", "label": "ユーザーID", "placeholder": "", "required": True, "element_id": ""}], "current_url": "https://example.com/login", "error": ""})
    proc = MagicMock()
    proc.stdout = payload
    proc.returncode = 0
    with patch.object(login_mod.subprocess, "run", return_value=proc):
        data = _client().post("/api/login/scrape", data={"url": "https://example.com/login", "domain": "example.com"}).get_json()
...
def test_scrape_rejects_invalid_domain(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(login_mod, "OUTPUT_DIR", tmp_path)
    res = _client().post("/api/login/scrape", data={"url": "https://x.com/login", "domain": "../etc"})
    assert res.status_code == 400
...
def test_scrape_timeout_returns_504(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(login_mod, "OUTPUT_DIR", tmp_path)
    import subprocess
    with patch.object(login_mod.subprocess, "run", side_effect=subprocess.TimeoutExpired("cmd", 30)):
        res = _client().post("/api/login/scrape", data={"url": "https://x.com/login", "domain": "x.com"})
...
def test_submit_success_saves_session(tmp_path: Path, monkeypatch) -> None:
    ...
    payload = json.dumps({"success": True, "needs_more_fields": False, "fields": [], "current_url": "https://example.com/", "error": ""})
    proc = MagicMock()
    proc.stdout = payload
    proc.returncode = 0
    with patch.object(login_mod.subprocess, "run", return_value=proc):
        data = _client().post("/api/login/submit", data={
            "domain": "example.com",
...
def test_submit_mfa_returns_new_fields(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(login_mod, "OUTPUT_DIR", tmp_path)
    payload = json.dumps({"success": False, "needs_more_fields": True, "fields": [{"name": "otp", "field_type": "text", "label": "OTP", "placeholder": "", "required": True, "element_id": ""}], "current_url": "https://example.com/mfa", "error": ""})
    proc = MagicMock()
    proc.stdout = payload
    proc.returncode = 0
    with patch.object(login_mod.subprocess, "run", return_value=proc):
...
def test_submit_rejects_missing_domain(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(login_mod, "OUTPUT_DIR", tmp_path)
    res = _client().post("/api/login/submit", data={"current_url": "https://x.com/login", "fields_json": "{}"})
    assert res.status_code == 400


def test_submit_rejects_invalid_json(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(login_mod, "OUTPUT_DIR", tmp_path)
    res = _client().post("/api/login/submit", data={"domain": "x.com", "current_url": "https://x.com/login", "fields_json": "not-json"})
    assert res.status_code == 400


def test_simple_login_success(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(login_mod, "OUTPUT_DIR", tmp_path)
    payload = json.dumps({"success": True, "needs_more_fields": False, "fields": [], "current_url": "https://x.com/", "error": ""})
    proc = MagicMock()
    proc.stdout = payload
    proc.returncode = 0
    with patch.object(login_mod.subprocess, "run", return_value=proc):
        data = _client().post("/api/login/simple", data={
            "domain": "x.com", "login_url": "https://x.com/login",
...
def test_simple_login_credentials_via_stdin(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_run(cmd, input=None, **kwargs):
        ...
        captured["cmd"] = cmd
        return MagicMock(stdout=json.dumps({"success": True, "needs_more_fields": False, "fields": [], "current_url": "", "error": ""}))
    with patch.object(login_mod.subprocess, "run", side_effect=fake_run):
        _client().post("/api/login/simple", data={
            "domain": "x.com", "login_url": "https://x.com/login",
...
def test_simple_login_rejects_missing_login_url(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(login_mod, "OUTPUT_DIR", tmp_path)
    res = _client().post("/api/login/simple", data={"domain": "x.com", "username": "u", "password": "p"})
    assert res.status_code == 400
...
def test_submit_fields_passed_via_stdin(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_run(cmd, input=None, **kwargs):
        ...
        captured["cmd"] = cmd
        proc = MagicMock()
        proc.stdout = json.dumps({"success": True, "needs_more_fields": False, "fields": [], "current_url": "", "error": ""})
        return proc
    with patch.object(login_mod.subprocess, "run", side_effect=fake_run):
        _client().post("/api/login/submit", data={
            "domain": "x.com",
