...
1. **P1-7 の OpenAPI 生成方式** — 自前生成（依存追加なし）で進めてよいか、apispec を入れるか
2. **証跡パックの出力形式** — Markdown + HTML の2種で足りるか（PDF が検収要件なら本弾に含める）
3. **無視ルールの設定UI** — 本弾はファイル（`diff_ignore.json`）のみとし、UI は次弾送りでよいか
