...
| `--output` | ./output | 出力先 |
| `--format` | md | md,html,excel をカンマ区切り |
...
| `report.html` | テストベース文書（サイドバー型・画面別カード＋テスト条件） |
...
- 完了: クローラー / ドリフト検知 / ログイン(CLI) / GUI多画面化 / テストベースレポート(Phase A) / レポートデザイン統一。
- 次: M1② PDF出力 → M1③ JSON出力 → M2 Phase B(自動化ヒント/メタ) → LLMテスト観点(最後) → M3 連携。
