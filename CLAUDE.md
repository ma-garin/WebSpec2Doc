...
├── .github/workflows/  ← CI（lint/型/テスト/カバレッジ/スモーク + secret scan）
├── tests/              ← テストスイート（pytest 162件）
...
pip install -r requirements-dev.txt   # ランタイム + ruff/black/mypy/pip-audit
...
- `scripts/verify.sh`：検証ゲート（py_compile→ruff/black/mypy→pytest+カバレッジ80%→実クロール2回→成果物チェック）。commit 前に必ず実行
- コミット前は `bash scripts/verify.sh` が `ALL GREEN` であることを確認

## CI とブランチ運用

- `.github/workflows/ci.yml` が push / PR ごとに verify 相当（ruff・black・mypy・pytest+カバレッジ80%・スモーク）と gitleaks secret scan を実行
- **AI（Claude/Codex）の作業はブランチ → PR で進め、main 直 push は避ける**。GitHub 側で main にブランチ保護（CI 必須・PR 必須）を設定するのを推奨
- ローカルの `verify.sh` と CI は同じゲートを共有する（ローカルで green なら CI も green になる設計）
