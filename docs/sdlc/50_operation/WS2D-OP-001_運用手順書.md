...
| `WEBSPEC2DOC_AUTH` | OFF | 利用者ログイン＋ワークスペースの有効化 |
| `WEBSPEC2DOC_SECRET_KEY` | 自動生成 | セッション署名鍵（未設定時 `instance/auth/secret.key`） |
...
export WEBSPEC2DOC_AUTH=1
export WEBSPEC2DOC_SECRET_KEY="<32+ 文字のランダム値>"
python app.py
```
初回ログイン時、既定ワークスペース `My Workspace` が自動作成される。
詳細は `docs/design/auth-tenant-integration.md`。
