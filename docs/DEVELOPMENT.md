...
4. 本書 §1「プロダクトの本質」 — なぜ作るか（稼働中システムの仕様空白を URL 一つで埋め、QA が初日からテスト設計に入れる状態を作る）
5. `docs/README.md` — 全文書の索引（正/歴史のステータス付き）
6. `docs/sdlc/README.md` — SDLC 文書体系（要件定義〜テストサマリ・V字・as-built・実測）
...
URL
 └─ crawler.page_crawler.crawl_site(url, depth, max_pages, output_dir, auth_state)
      ...
      ├─ politeness: robots.txt 尊重・per-origin レート制御（既定で有効）
      ├─ network_interceptor: 破壊的リクエスト（POST/DELETE等）の遮断・XHR観測
      ├─ action_explorer + spa_monitor: モーダル・タブ等の隠れ状態を開いて記録（SPA対応）
      ├─ link_extractor: リンク / フォーム(+制約+選択肢) / ボタン / 見出し / タイトル
      ├─ response / console / mixed-content / axe-core の実測を PageData に保持
 ...
 └─ health.technical_health   → technical_health.json（到達済み・同一オリジンの実測範囲）
 └─ generator.accessibility_reporter → accessibility_audit.json（機械判定のみ、手動確認要）
 └─ generator.*               → screens.md / forms.md / transition.mmd / report.html / spec.xlsx
 └─ diff.snapshot.save_snapshot(pages) → snapshots/{ts}.json
 └─ (--compare) diff.differ.compute_diff(old,new) → diff_reporter
      └─ diff_report.html / diff_summary.json（通知向け上位N件サマリー）
```
...
| GUI の画面/設定 | `app.py` はエントリーポイント（Flask + `web/` パッケージ）。**4ステップウィザード**（①解析: URL入力＋画面分析 → ②条件設定: 画面選択＋個別ログインフォーム＋差分ON/OFF → ③実行: ライブプレビュー＋4段階進捗バー → ④レポート）。`/api/discover` で画面一覧取得（タイムアウト180秒）、認証必要画面は per-page インラインフォームで個別ログイン（`/api/login/simple`）。結果配信は `/api/result`（生成物一覧＋サマリー）`/preview`（iframe・テキスト）`/download`・`/download-zip`（パスガード付き）。運用設定は `web/routes/schedule.py` と `web/services/scheduler.py`（曜日・時間帯・タイムゾーン・再試行・実行履歴・通知）に集約し、通知先は書き込み専用でAPIレスポンスへ返さない。初回ツアーは `static/js/onboarding.js` が担当し、チェックリストはツアー中だけ表示する。静的ファイルは `static/js/`＋`static/app-view.js`＋`static/app.css`、テンプレートは `templates/index.html`＋`templates/partials/`。 |
...
| AutoRun | `web/routes/auto_run.py`。URL駆動と文書駆動を同じ実行フローで扱う。文書駆動は crawl → QA生成の後に `src/mbt/` で要件追跡・パス選定・手動手順・テストデータ・任意の非送信バリデーション観測を生成してから spec.ts 生成へ進む。ジョブ管理は `AutoRunJob` dataclass、ログ/状態はポーリング（`/api/autorun/status`）。 |
...
bash scripts/verify.sh   # py_compile + 800行ガード + ruff/black/mypy + 全ユニットテスト + スモーク

# 個別実行（デバッグ用）
python -m py_compile app.py $(find src web -name '*.py' -not -path '*__pycache__*')
python -m pytest tests/ -q                       # 全件緑が基準（2026-07 時点 1,194 本）
...
800行ガードは、新規ファイルを一律800行以下に制限する。既に超過している既知ファイルは
`scripts/verify.sh` の `PHASE1_ALLOWED_MAX` にファイル別上限を固定し、現状以上の肥大化を
同じゲートで拒否する。分割できたファイルは上限リストから削除する。
...
[完了] クローラー / ドリフト検知 / 自動ログイン(GUI) / GUI多画面化 /
       ...
       観点管理3ペインUI（ツリー分類・インライン編集・AI提案） /
       レポートUX刷新＋セキュリティ強化（Host/Origin検証） /
       品質レベルアップ3層（クロール礼儀・evidence根拠付与・状態探索・トレーサビリティ） /
       LLMテスト観点（src/llm/ ルールベース+OpenAI補完のprovider構成） /
       実機デモ基盤（demo/ DemoMart・make demo）/ ROIダッシュボード（/usage） /
       テスト1,194本＋E2E 64本・コア90%+

★ v1.0 完成済み。今後の拡張は docs/11_機能拡張ロードマップ_現新比較とUX検証.md が正
  （Sprint A: iframe/Shadow DOM 対応が次の着手予定）。
```

プロダクトの目的と今後の拡張方針は、本書および `docs/11_機能拡張ロードマップ_現新比較とUX検証.md` を参照。
...
- **Codex 連携**: 仕様が固まった実装の委任先。セッションは必ず正本ディレクトリ `/Users/fujimagariyuki/dev/active/webspec2doc` で開く（旧コピーは統合済み・参照禁止）。委任後は成果物を必ずレビュー（過去に未配線スタブ混入の事例あり）。
