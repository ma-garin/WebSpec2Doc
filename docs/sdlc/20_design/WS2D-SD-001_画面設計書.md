...
| auto-run | AutoRun | `/auto-run` | URL→全自動テスト。**生成モード（URL駆動/文書駆動）**を切替。文書駆動時のみ「要件・仕様文書」「パス選定基準（頂点網羅/エッジ網羅/プライムパス/到達目標）」「入力検証の観測」を表示。承認モーダル（実行対象/デバイス/制限時間・出力形式（フラット/Page Object）、文書駆動時は要件数・対応画面数・選定パス数・カバー率） |
