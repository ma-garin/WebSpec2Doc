...
class TestGenerateAndApprove:
    ...
    def test_rejects_unknown_stage(self, client) -> None:
        res = _post(client, "/api/autorun/stages/generate", {"domain": DOMAIN, "stage_id": "nope"})
...
class TestFeatureApprovalGate:
    ...
    def test_blocks_approval_while_items_unapproved(self, client) -> None:
        ...
        res = _post(
            client, "/api/autorun/stages/approve", {"domain": DOMAIN, "stage_id": "features"}
        )
...
class TestSkipRules:
    ...
    def test_first_run_cannot_skip(self, client) -> None:
        res = _post(client, "/api/autorun/stages/skip", {"domain": DOMAIN, "stage_id": "test_plan"})
        assert res.status_code == 409

    def test_non_skippable_stage_is_rejected(self, client) -> None:
        res = _post(client, "/api/autorun/stages/skip", {"domain": DOMAIN, "stage_id": "features"})
        assert res.status_code == 400
...
class TestItemEditing:
    def test_editing_marks_item_as_user_edited(self, client) -> None:
        ...
        updated = next(
            i
            for s in res.get_json()["stages"]
            if s["stage_id"] == "test_objective"
            for i in s["items"]
...
class TestSuggestAndAdopt:
    ...
    def test_suggest_without_llm_is_not_an_error(self, client, monkeypatch) -> None:
        ...
        res = _post(
            client, "/api/autorun/stages/suggest", {"domain": DOMAIN, "stage_id": "test_objective"}
        )
        assert res.status_code == 200
        assert res.get_json()["available"] is False

    def test_suggest_rejects_unknown_stage(self, client) -> None:
        res = _post(client, "/api/autorun/stages/suggest", {"domain": DOMAIN, "stage_id": "nope"})
        assert res.status_code == 400
...
class TestExecutionGate:
    ...
    def _approve_all(self, client) -> None:
        ...
        for stage_id in order:
            body = _post(
                client, "/api/autorun/stages/generate", {"domain": DOMAIN, "stage_id": stage_id}
            ).get_json()
