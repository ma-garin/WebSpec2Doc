...
| GUI の画面/設定 | `app.py`（単一ファイル・Flask + インラインHTML/CSS/JS、**UI変更フロー厳守**） |
...
python -m pytest tests/ -q                       # 158本 緑が基準
...
M1② PDF出力       : report.html を Playwright(page.pdf)で report.pdf 化。--format pdf 追加。
                    印刷CSS(@media print)済み。mermaidはCDN描画待ちが要る点に注意。
M1③ JSON出力      : snapshot を出力形式に昇格（--format json）。GUI/履歴に出力ボタン。
M2  レポートPhase B: ⑤自動化ヒント（name/id ロケータ候補）⑥メタ拡充（クロール条件の記録）。
...
★ v1.0 の線 = M0 + M1(①②③) + ドキュメント更新。LLM/連携は v1.1 以降。
```
