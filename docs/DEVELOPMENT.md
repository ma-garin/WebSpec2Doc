...
bash scripts/verify.sh   # py_compile + 800行ガード + ruff/black/mypy + pytest 313本 + スモーク

# 個別実行（デバッグ用）
python -m py_compile app.py $(find src web -name '*.py' -not -path '*__pycache__*')
python -m pytest tests/ -q                       # 313本 緑が基準
...
[完了] クローラー / ドリフト検知 / ログイン(CLI) / GUI多画面化 /
       ...
       コードベース整備（app.py→web/分割・巨大ファイル3本解消・800行ガード・CI web/拡張）
