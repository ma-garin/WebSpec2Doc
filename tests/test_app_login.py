...
import web.process as proc_mod
...
def test_login_start_spawns_subprocess(tmp_path: Path, monkeypatch) -> None:
    ...
    proc_mod._LOGIN_PROCS.clear()
    with patch.object(login_mod.subprocess, "Popen", return_value=MagicMock()) as popen:
        data = (
            _client()
            .post(
                "/api/login/start",
    ...
    assert popen.called
    assert "example.com" in proc_mod._LOGIN_PROCS
...
def test_login_finish_writes_signal_and_reports_saved(tmp_path: Path, monkeypatch) -> None:
    ...
    proc.returncode = 0
    proc_mod._LOGIN_PROCS["example.com"] = proc
